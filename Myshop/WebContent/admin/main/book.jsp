	...
	<body style="background: url(<%=path %>/admin/img/mbg.jpg);">
		<div class="admin-in">
			...
			<table class="datatable" width="1150" height="auto" border="1">
				...
				<jsp:useBean id="PageBook" scope="request" class="page.bookPage"/>
				...
				<%
					String s=String.valueOf(PageBook.getCurPage());
					ArrayList list1=PageBook.getResult(s).data;
		...
		<style>
			...
			<div class="page">
				 <%if(PageBook.getMaxPage()!=1){ %>
				<form name="PageForm"  action="<%=path %>/BookpageServlet" method="post">
				每页
				<%=PageBook.rowsPerPage%>
				行
				第
				<%=PageBook.getCurPage()%>
				页
				共
				<%=PageBook.getMaxPage()%>页
				<%
				  if (PageBook.getCurPage() == 1) {
				    out.print(" 首页 上一页");
				  }
				  else {
				%>
				<a HREF="javascript:gotoPage(1)">首页</A>
				<a HREF="javascript:gotoPage(<%=PageBook.getCurPage()-1%>)">上一页</A>
				<%}%>
				<%
				  if (PageBook.getCurPage() == PageBook.getMaxPage()) {
				    out.print("下一页 尾页");
				  }
				  else {
				%>
				<a HREF="javascript:gotoPage(<%=PageBook.getCurPage()+1%>)">下一页</A>
				<a HREF="javascript:gotoPage(<%=PageBook.getMaxPage()%>)">尾页</A>
				<%}%>
				转到第
				<SELECT name="jumpPage" onchange="Jumping()">
				<%
				  for (int i = 1; i <= PageBook.getMaxPage(); i++) {
				    if (i == PageBook.getCurPage()) {
				%>
