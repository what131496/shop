	...
	<div class="car-main commonWidth">
		<div class="car-title">
		   ...
		   <%
	    	...
	    	%>
	    	<form action="<%=path %>/IndentServlet?method=add" method="post">
		<div class="commonWidth buy-total">
			<div class="total-title clearfix">
				<h4>订单提交成功，请您尽快付款</h4>
				<h6>请您尽快在<span id="LeftTime"></span>之后付款。否则我们会取消订单。（库存紧张，仅以订单为准）</h6>
			         ...
			         <div id="hpn" class="hpn" style="display:none">
			           ...
			           <%
					    ...
					    %>
			             ...
			             <input type="hidden" name="address" value="<%=address.getAddress() %>"> 
			              <input type="hidden" name="phone" value="<%=address.getPhone() %>"> 
			           </p>
			           <%} %>
			           <p>商品名称：
			             <table width="800">
	    		...
	    		%>
						   	<tr>
						   	 ...
						   	 <input type="hidden" name="booknum" value="<%=goods.get(b)%>">
						  	<%} %>
						   </table>
			         </div>
			     </div>
			</div>
			<div class="total-content" id="select">
				<h4>请选择支付方式：</h4>
				<ul class="">
					...
					<li class="payhover"> 微信<img src="../img/cart/weixin.jpg"></li>
					 <% 
               CardDao itemsDao = new CardDao(); 
               ArrayList<Card> list = itemsDao.getAllCard();
           ...
           %>   
					<li class="payhover"> <%=item.getCard() %>（<%=item.getCardNumber() %>）<img src="../img/cart/nonghang.jpg"></li>
				    <%}
	               } %>
				</ul>
			      ...
			      <div class="theme-popbod dform">
			          <form class="theme-signin" name="loginform" action="../../CardServlet" method="post">
			                <ol>
			                     ...
			                     <li><strong>开卡银行：</strong><input class="ipt" type="text" name="card" value="" size="20" /></li>
			                     <li><strong>银行卡号：</strong><input class="ipt" type="text" name="cardnumber" value="" size="20" /></li>
			...
			<div class="add-ic">
				<input class="fr" type="submit" value="立即支付" />
			</div>
		</div>
		</form>
	</div>
